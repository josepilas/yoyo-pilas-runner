...
function renderList() {
  visibleGames = [...games];
  gameCount.textContent = `${visibleGames.length} game${visibleGames.length === 1 ? "" : "s"}`;
...
function renderDetails() {
  ...
  cover.style.backgroundImage = `linear-gradient(135deg, ${game.color}33, rgba(4, 12, 7, 0.74)), url("../assets/logo.webp")`;
}
...
document.addEventListener("keydown", (event) => {
  ...
  if (event.key === "Escape" || event.key === "Backspace") {
    setActivity("Exit", "back", "The native runner would return to PortMaster.", 0);
  }
});
